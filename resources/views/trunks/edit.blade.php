...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-900">Edit Trunk: {{ $trunk->trunk_name }}</h1>
            ...
            <form method="POST" action="{{ route('trunks.update', $trunk) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="trunk_name" name="trunk_name" value="{{ old('trunk_name', $trunk->trunk_name) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" id="provider" name="provider" value="{{ old('provider', $trunk->provider) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('provider')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="password" id="password" name="password" value="{{ old('password', $trunk->password) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('password')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
